...
sorted_words=`curl -s $1|tr [A-Z] [a-z]|sed -e '/[^[:alpha:]]//g'|grep -oE "\w+"|sort|uniq -c|sort -k1,1 -k2r| sort -nr`
echo $sorted_words
...
echo "Min frequency and word $(echo "$sorted_words"|tail -n1) "
echo "Max frequency and word  $(echo "$sorted_words"|head -n1)"
...
declare -a myArray=()
for w in $sorted_words; do myArray+=($w);done
if (($total_uniq_words % 2 ==0))
then
    midpos=$(($total_uniq_words/2))
    midpos2=$((midpos+1))
    echo ${myArray[$midpos]}
    echo "Even"
    
else
    midpos=$((($total_uniq_words+1)/2))
    echo $midpos
    echo "Odd"
fi
